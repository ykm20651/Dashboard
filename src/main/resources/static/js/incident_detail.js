document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadIncident() {
    try {
      ...
      const res = await fetch(`http://52.79.99.132:8080/incidents/${incidentId}`, {
  ...
  async function loadEvidence(editMode = false) {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`http://52.79.99.132:8080/incidents/${incidentId}/evidence-files`, {
        headers: { "Authorization": `Bearer ${token}` }
      });
  ...
  deleteSelectedBtn.addEventListener("click", async () => {
    ...
    for (let box of checked) {
      const res = await fetch(`http://52.79.99.132:8080/incidents/${incidentId}/evidence-files/${box.value}`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${token}` }
      });
